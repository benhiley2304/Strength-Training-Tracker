...
const payload = state => ({theme: state.theme, profiles: state.profiles.map(p => ({...p,
  // Rendering prepares blank drafts with random IDs. They are not user edits.
  drafts: Object.fromEntries(Object.entries(p.drafts).filter(([, d]) => hasEffort(d, p.bw)))
}))});
export const sameState = (a, b) => !!a && !!b && equalData(payload(a), payload(b));
...
export function hasEffort(d, defaultBW = null) {
  return !!d && (d.bwSnapshot !== defaultBW || !!d.startedAt || !!d.restEndAt || !!d.notes || d.conditioning !== 'None' || Object.values(d.readiness).some(x => x !== null) || d.exercises.some((e, i) => e.sets.length !== sessions[d.session - 1].ex[i].sets || e.sets.some(s => s.done || s.kg !== '' || s.reps !== '' || s.rpe !== '')));
}
export function reconcile(base, local, remote, {replacement = false, sameRevision = false} = {}) {
  ...
  const eligible = (d, bd, defaultBW) => {
    ...
    if (finishes.length && !(hasEffort(d, defaultBW) && d.startedAt && finishes.every(h => d.startedAt > Date.parse(h.date)) && bd?.id !== d.id)) return undefined;
    return d;
  };
  for (const key of new Set([...Object.keys(l.drafts), ...Object.keys(r.drafts)])) {
    const bd = b?.drafts[key], ld = eligible(l.drafts[key], bd, l.bw), rd = eligible(r.drafts[key], bd, r.bw);
    let d;
    if (ld && rd && ld.id !== rd.id) {
      if (!hasEffort(ld, l.bw)) d = copy(rd);
      else if (!hasEffort(rd, r.bw)) d = copy(ld);
