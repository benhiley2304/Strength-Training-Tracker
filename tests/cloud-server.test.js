...
import {newDraft, changeExercise, completedSets} from '../model.js';
...
test('substitutions persist with actual set identities and obsolete clients cannot erase them', async t => {
  const {call, signup} = await fixture(t); const a = await signup();
  const profile = a.data.profile;
  profile.drafts[1] = newDraft(profile);
  profile.drafts[1].exercises[0].sets[0] = {kg: '80', reps: '4', rpe: '', done: true};
  changeExercise(profile.drafts[1], 0, 'Lat Pulldown');
  const body = {revision: 0, profile, theme: 'light'};
  assert.equal((await call('/api/profile', {method: 'PUT', cookie: a.cookie, body})).status, 426);
  const saved = await call('/api/profile', {method: 'PUT', cookie: a.cookie, body, headers: {'X-Tracker-Features': 'exercise-substitutions-v1'}});
  assert.equal(saved.status, 200);
  assert.equal(saved.data.profile.exerciseSchemaVersion, 1);
  assert.equal(saved.data.profile.drafts[1].exercises[0].name, 'Lat Pulldown');
  assert.equal(completedSets(saved.data.profile.drafts[1])[0].exercise, 'Bench Press');
  const downgrade = structuredClone(saved.data.profile);
  delete downgrade.exerciseSchemaVersion;
  downgrade.drafts = {};
  assert.equal((await call('/api/profile', {method: 'PUT', cookie: a.cookie, body: {revision: 1, profile: downgrade, theme: 'light'}})).status, 426);
  assert.equal((await call('/api/account', {cookie: a.cookie})).data.profile.drafts[1].exercises[0].name, 'Lat Pulldown');
});
